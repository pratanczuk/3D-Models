...
total_width = 65;
/* width of each side in mm */
side_width = 10;
/* depth of the cross bars in mm */
bar_depth = 10;

empty_space = total_width - (2*side_width);


rotate([90,0,0])
 difference() {
linear_extrude(file = "phone_stand_pst-s.dxf", height = total_width);
#translate([0,7,side_width]) cube([100,100,empty_space]);
#translate([bar_depth,0,side_width]) cube([72-(2*bar_depth),100,empty_space]);
};

//Draw text
rotate([0,0,90]) translate([-47,-70.5,7]) scale([1,1,2]) drawtext("PST-S");
