...
inch=25.4;
set_screw_dia=4.75;
shaft_collar_radius=(3/8*inch+0.9)/2;
shaft_collar_depth=9;
use_shaft_collar=false;
...
module WadeL_double_helix(){
	...
	height=11;
	pressure_angle=30;
        hub_dia = use_shaft_collar ? 25 : 20;
	...
	difference(){
		...
		gear (number_of_teeth=teeth,
			...
			clearance = 0.2,
			gear_thickness =0,// height/2*0.25,
			rim_thickness = height/2,
			rim_width = 5,
			hub_thickness = height/2*1.5,
			hub_diameter = hub_dia,
			bore_diameter = 1/8 * inch + 0.25,
			circles=circles,
			twist = twist/teeth);
		mirror([0,0,1])
		gear (number_of_teeth=teeth,
			...
			hub_diameter=hub_dia,
			bore_diameter=1/8 * inch + 0.5,
			circles=circles,
			twist=twist/teeth);
		}
		if (use_shaft_collar) {
		  translate([0,0,3]) rotate([180,0,0]) {
		    cylinder(100, 1/16 * inch, center=true);
		    translate([0,0, shaft_collar_depth -height * 0.5])
		      rotate([180,0,0])
		      cylinder(20, shaft_collar_radius, shaft_collar_radius);
		  }
		  translate([0,set_screw_dia/2,1])
		    rotate([90,0, 0])
                    cube([20,8,set_screw_dia]);
		} else {
                  for (angle=[90,-90]) {
                    rotate([0,0,angle]) {
#                     translate([0,-5,height/4*1.5])cube([5.7,2.3,9],center = true);
#                     translate([0, 0,height/4*1.5])rotate([0,90,-90])cylinder(r=1.7,h=20);
                    }
                  }
		}
	}
...
module WadesS_double_helix(){
	...
	difference(){
		...
		translate([0,-5,12])cube([5.7,2.3,9],center = true);
